...
function SalaEsperaPage() {
  ...
  const fetchDetallesPartida = useCallback(async () => {
    try {
      ...
      if (data.estado === 'en_progreso') {
        console.log("La partida ya comenzó.");
      }
  ...
  useEffect(() => {
    ...
    alert('¡La partida ha comenzado! Cargando tablero...');
      navigate(`/juego/${data.partidaId}`); 
  });
  ...
  const handleIniciarPartida = async () => {
    setError(null);
    try {
      const response = await fetch(`http://localhost:3000/partidas/${partidaId}/iniciar`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
